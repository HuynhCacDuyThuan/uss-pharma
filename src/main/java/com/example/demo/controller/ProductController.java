...
public class ProductController {
    ...
    public String addProduct(
        ...
        // Create product object from the provided ProductDTO
        ...
        for (MultipartFile multipartFile : files) {
            if (!multipartFile.isEmpty()) {  // Kiểm tra nếu file không rỗng
                ...
                productImageRepository.save(productImage);  // Lưu ảnh vào cơ sở dữ liệu
            }
        }
    ...
    public ResponseEntity<?> showEditProductForm(@PathVariable("id") Long id) {
    ...
    public String editProduct(
        ...
        if (files != null || !files.isEmpty()) {
            // Fetch existing product images to check if there are any already in the database
            List<ProductImage> existingImages = productImageRepository.findByProduct(product);

            // Create a list to store names of the uploaded files (new ones)
            List<String> uploadedImageNames = new ArrayList<>();

            // Iterate over the uploaded files
            for (MultipartFile multipartFile : files) {
                if (!multipartFile.isEmpty()) {  // Check if the file is not empty
                    // Generate a unique file name for the additional image
                    ...
                    // Create the file object for storing the image
                    File imageFile = new File(uploadDir, additionalImageFileName);
                    try (FileOutputStream imageFos = new FileOutputStream(imageFile)) {
                        imageFos.write(multipartFile.getBytes());
                    }
            ...
            // Delete any old images that are not in the new uploaded list
            for (ProductImage existingImage : existingImages) {
                if (!uploadedImageNames.contains(existingImage.getImageUrl())) {
                    // If the image is not in the list of new uploaded images, delete it
                    productImageRepository.delete(existingImage);  // Remove from DB

                    // Optionally, delete the image file from the file system
                    File oldImageFile = new File(uploadDir, existingImage.getImageUrl());
                    if (oldImageFile.exists()) {
                        oldImageFile.delete();  // Delete the image file
                    }
                }
            }
        }
